...
fuProgressDeath <- clinData %>% filter(is.na(date_progression) & !is.na(date_death)) %>% mutate(time_fu_prog = as.double(difftime(date_death, date_liver_op, units = c("days"))), status_fu_prog = 1)
fuProgFree <- clinData %>% filter(is.na(date_progression) & is.na(date_death) & is.na(date_lost_follow_up)) %>% mutate(time_fu_prog = as.double(difftime(dateEndFu, date_liver_op, units = c("days"))), status_fu_prog = 0)
fuProgLost <- clinData %>% filter(is.na(date_progression) & !is.na(date_lost_follow_up) & is.na(date_death)) %>% mutate(time_fu_prog = as.double(difftime(date_lost_follow_up, date_liver_op, units = c("days"))), status_fu_prog = 0) # "T7303/2015" lost fu 2015 but dod known 2016
...
#calculate PFS with date_progression_liver
fuProgressLiver <- clinData %>% filter(!is.na(date_progression_liver)) %>% mutate(time_fu_prog_liv = as.double(difftime(date_progression_liver, date_liver_op, units = c("days"))), status_fu_prog_liv = 1)
fuProgFreeLiver <- clinData %>% filter(is.na(date_progression_liver) & is.na(date_lost_follow_up)) %>% mutate(time_fu_prog_liv = as.double(difftime(dateEndFu, date_liver_op, units = c("days"))), status_fu_prog_liv = 0)
fuProgLostLiver <- clinData %>% filter(is.na(date_progression_liver) & !is.na(date_lost_follow_up)) %>% mutate(time_fu_prog_liv = as.double(difftime(date_lost_follow_up, date_liver_op, units = c("days"))), status_fu_prog_liv = 0)

clinSurvDataPFSliv <- rbind(fuProgressLiver, fuProgFreeLiver, fuProgLostLiver)
...
clinSurvData <- merge(clinSurvData, select(clinSurvDataPFSliv, pad, time_fu_prog_liv, status_fu_prog_liv))
