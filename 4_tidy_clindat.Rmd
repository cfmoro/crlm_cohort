...
```{r}
clinDataFn <- "./datasets/KS_CRLM_2012_2025_pat_260_20211212.xlsx" # "CRLM_clin_cohort_210315.xlsx"
patDataFn1 <- "./datasets/CRLM_KS_cohort_pathology_Evelina.xlsx" # Evelina: 1-58 # CRLM_KS_cohort_pathology_Evelina (200904) # Lorand (210309)
patDataFn2 <- "./datasets/CRLM_KS_cohort_pathology_Media-Yousra.xlsx" # Lorand/Danyil-Media-Yousra: 59-310 # "Lorand"
...
dateEndFu <- as.Date("2021-12-12") #  Next: "2021-03-08" # Initial Evelina's cohort as.Date("2020-10-29") # Current date of end follow up for study
...
clinData <- read_excel(clinDataFn, col_types = c("numeric", "skip", "date", "text", "text", "date", "date", "text",  "date", "date", "text", "text", "date"))
# Issues in original file: Expecting date in J65 / R65C10: got 'U'Expecting date in F196 / R196C6: got '1014-09-08'


# Rename columns with easy and concise names: rename(flights, tail_num = tailnum)
(nms <- names(clinData))
clinData <- rename(clinData, id = "PaRaMet nr", date_birth = dob, primary_side = "location of primary (left/right)",
...
fuProgLost <- clinData %>% filter(is.na(date_progression) & !is.na(date_lost_follow_up)) %>% mutate(time_fu_prog = as.double(difftime(date_lost_follow_up, date_liver_op, units = c("days"))), status_fu_prog = 0)
