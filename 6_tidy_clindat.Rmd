...
clinDataFn <- paste(workDir, "datasets/CRLM_clin_cohort_210315.xlsx", sep="")
patDataFn1 <- paste(workDir, "datasets/CRLM_KS_cohort_pathology_Evelina.xlsx", sep="") # Evelina: CRLM_KS_cohort_pathology_Evelina (200904) # Lorand (210309)
patDataFn2 <- paste(workDir, "datasets/CRLM_KS_cohort_pathology_Lorand.xlsx", sep="")
...
dateEndFu <- as.Date("2021-03-08") # Initial Evelina's cohort as.Date("2020-10-29") # Current date of end follow up for study
...
# Read excel file
clinData <- read_excel(clinDataFn, col_types = c(
  "numeric", "date", "text", "skip", "text", "skip", "skip", "skip", "skip", "skip",
  "skip", "date", "skip", "skip", "skip", "skip", "skip", "date", "text", "skip",
  "date", "skip", "date", "skip", "skip", "text", "text", "skip", "skip", "skip",
  rep("skip", 16), "date", rep("skip",46)))

# Rename columns with easy and concise names: rename(flights, tail_num = tailnum)
(nms <- names(clinData))
clinData <- rename(clinData, id = "PaRaMet pseudonym", date_birth = dob, primary_side = "location of primary (left/right)",
...
# Filter Evelina or Danyil cohort
#clinData <- clinData %>% filter(id < 60) # Evelina
# Remove wrong rows
...
fuLostOS <- clinData %>% filter(!is.na(date_lost_follow_up)) %>% mutate(time_fu_os = as.double(difftime(date_lost_follow_up, date_liver_op, units = c("days"))), status_fu_os = 0)

fuProgress <- clinData %>% filter(!is.na(date_progression)) %>% mutate(time_fu_prog = as.double(difftime(date_progression, date_liver_op, units = c("days"))), status_fu_prog = 1)
fuProgFree <- clinData %>% filter(is.na(date_progression) & is.na(date_lost_follow_up)) %>% mutate(time_fu_prog = as.double(difftime(dateEndFu, date_liver_op, units = c("days"))), status_fu_prog = 0)
fuProgLost <- clinData %>% filter(!is.na(date_lost_follow_up)) %>% mutate(time_fu_prog = as.double(difftime(date_lost_follow_up, date_liver_op, units = c("days"))), status_fu_prog = 0)

clinSurvDataOS <- rbind(fuDeath, fuAlive, fuLostOS)
clinSurvDataPFS <- rbind(fuProgress, fuProgFree, fuProgLost)

clinSurvData <- merge(clinSurvDataOS, select(clinSurvDataPFS, id, time_fu_prog, status_fu_prog))
...
patDataOrig1 <- read_excel(patDataFn1, col_types = c(
  "skip", "numeric", "skip", "numeric", "text",  rep("skip", 8),
...
for(a_letter in tumor_letters) {
  ...
  paired_columns <- patDataTemp %>% select(projectnr, {{largest_diameter_col_name}}, {{percent_tumor_col_name}}) %>% filter(!is.na(.data[[largest_diameter_col_name]]))
  ...
  #if(a_letter == 'a') {
  #  for(an_id in incomplete_cases$projectnr) {
  #    ids_incomplete_tumor_report_a <- c(ids_incomplete_tumor_report_a, an_id)
  #  } 
  #}
}
#ids_incomplete_tumor_report <- unique(ids_incomplete_tumor_report)
...
#patDataTemp <- patDataTemp %>%  mutate(sum_tumour_score_report_old = sum(
#  `tumor_a_percent_tumor (%)` * `tumor_a_largest_diameter (cm)`, `tumor_b_percent_tumor (%)` * `tumor_b_largest_diameter (cm)`,
#  `tumor_c_percent_tumor (%)` * `tumor_c_largest_diameter (cm)`, `tumor_d_percent_tumor (%)` * `tumor_d_largest_diameter (cm)`,
#  `tumor_e_percent_tumor (%)` * `tumor_e_largest_diameter (cm)`, `tumor_f_percent_tumor (%)` * `tumor_f_largest_diameter (cm)`,
#  `tumor_g_percent_tumor (%)` * `tumor_g_largest_diameter (cm)`, `tumor_h_percent_tumor (%)` * `tumor_h_largest_diameter (cm)`,
#  `tumor_i_percent_tumor (%)` * `tumor_i_largest_diameter (cm)`, `tumor_j_percent_tumor (%)` * `tumor_j_largest_diameter (cm)`,
#  `tumor_k_percent_tumor (%)` * `tumor_k_largest_diameter (cm)`, `tumor_l_percent_tumor (%)` * `tumor_l_largest_diameter (cm)`,
#  `tumor_m_percent_tumor (%)` * `tumor_m_largest_diameter (cm)`, `tumor_n_percent_tumor (%)` * `tumor_n_largest_diameter (cm)`,
#  `tumor_o_percent_tumor (%)` * `tumor_o_largest_diameter (cm)`, `tumor_p_percent_tumor (%)` * `tumor_p_largest_diameter (cm)`
#  , na.rm = TRUE))
...
#patDataTemp <-patDataTemp %>% mutate(sum_tumour_score_report_old = na_if(sum_tumour_score_report, 0.0))
# Now, remove values for those probes with incomplete tumor regression in report in tumor a
#index <- patDataTemp$projectnr %in% ids_incomplete_tumor_report_a
#patDataTemp$sum_tumour_score_report[index] <- NA

patDataTemp <- patDataTemp %>% relocate(sum_tumour_score_report, .before = `tumor_a_percent_tumor (%)`)
#patDataTemp <- patDataTemp %>% relocate(sum_tumour_score_report_old, .before = `tumor_a_percent_tumor (%)`)
...
# Classify each probe accordibng to predominant GP (4 cartegories)
...
# Classify each probe accordibng to predominant GP (3 cartegories)
...
for(a_letter in tumor_letters) {
  ...
  #if(a_letter == 'a') {
  #  for(an_id in incomplete_cases_annot$id) {
  #    ids_incomplete_tumor_annot_a <- c(ids_incomplete_tumor_annot_a, an_id)
  #  }
  #}
}
#ids_incomplete_tumor_annot <- unique(ids_incomplete_tumor_annot)
...
# Derive sum regression score per probe (sum of % tumor x diameter)
# TODO: sum of NA (or a product of it) is 0. We need to garantee every diameter is matched with percent tumour. Add integrity check code
#tumPercentByTumorDiams <- tumPercentByTumorDiams %>% rowwise() %>%  mutate(sum_tumour_score_annot_old = sum(
#  `%_tum_ann_a` * `tumor_a_largest_diameter (cm)`, `%_tum_ann_b` * `tumor_b_largest_diameter (cm)`,
#  `%_tum_ann_c` * `tumor_c_largest_diameter (cm)`, `%_tum_ann_d` * `tumor_d_largest_diameter (cm)`,
#  `%_tum_ann_e` * `tumor_e_largest_diameter (cm)`, `%_tum_ann_f` * `tumor_f_largest_diameter (cm)`,
#  `%_tum_ann_g` * `tumor_g_largest_diameter (cm)`, `%_tum_ann_h` * `tumor_h_largest_diameter (cm)`,
#  `%_tum_ann_i` * `tumor_i_largest_diameter (cm)`, `%_tum_ann_j` * `tumor_j_largest_diameter (cm)`,
#  `%_tum_ann_k` * `tumor_k_largest_diameter (cm)`, `%_tum_ann_l` * `tumor_l_largest_diameter (cm)`,
#  `%_tum_ann_m` * `tumor_m_largest_diameter (cm)`, `%_tum_ann_n` * `tumor_n_largest_diameter (cm)`,
#  `%_tum_ann_o` * `tumor_o_largest_diameter (cm)`, `%_tum_ann_p` * `tumor_p_largest_diameter (cm)`
#  , na.rm = TRUE))
...
# tumPercentByTumorDiams <- tumPercentByTumorDiams %>% relocate(sum_tumour_score_annot_old, .after = id)
...
# Check needed columns exist, create otherwise
#replacement_tum_ann_col_names <- c("replacement_tum_ann_g", "replacement_tum_ann_h", "replacement_tum_ann_i", "replacement_tum_ann_j", "replacement_tum_ann_k", #"replacement_tum_ann_l", "replacement_tum_ann_m", "replacement_tum_ann_n", "replacement_tum_ann_o", "replacement_tum_ann_p")
#for(a_col_name in replacement_tum_ann_col_names) {
#  if(!(a_col_name %in% colnames(replacementByTumorDiams))) {
#    replacementByTumorDiams <-replacementByTumorDiams %>% add_column({{a_col_name}} := NA) 
#  }
#}
...
for(a_letter in tumor_letters) {
  ...
  #if(a_letter == 'a') {
  #  for(an_id in incomplete_cases_annot$id) {
  #    ids_incomplete_replacement_annot_a <- c(ids_incomplete_replacement_annot_a, an_id)
  #  }
  #}
}
#ids_incomplete_replacement_annot <- unique(ids_incomplete_replacement_annot)
...
# Derive sum regression score per probe (sum of % tumor x diameter)
# TODO: sum of NA (or a product of it) is 0. We need to garantee every diameter is matched with percent tumour. Add integrity check code
#replacementByTumorDiams <- replacementByTumorDiams %>% rowwise() %>%  mutate(sum_replacement_score_annot_old = sum(
#  `replacement_tum_ann_a` * `tumor_a_largest_diameter (cm)`, `replacement_tum_ann_b` * `tumor_b_largest_diameter (cm)`,
#  `replacement_tum_ann_c` * `tumor_c_largest_diameter (cm)`, `replacement_tum_ann_d` * `tumor_d_largest_diameter (cm)`,
#  `replacement_tum_ann_e` * `tumor_e_largest_diameter (cm)`, `replacement_tum_ann_f` * `tumor_f_largest_diameter (cm)`,
#  `replacement_tum_ann_g` * `tumor_g_largest_diameter (cm)`, `replacement_tum_ann_h` * `tumor_h_largest_diameter (cm)`,
#  `replacement_tum_ann_i` * `tumor_i_largest_diameter (cm)`, `replacement_tum_ann_j` * `tumor_j_largest_diameter (cm)`,
#  `replacement_tum_ann_k` * `tumor_k_largest_diameter (cm)`, `replacement_tum_ann_l` * `tumor_l_largest_diameter (cm)`,
#  `replacement_tum_ann_m` * `tumor_m_largest_diameter (cm)`, `replacement_tum_ann_n` * `tumor_n_largest_diameter (cm)`,
#  `replacement_tum_ann_o` * `tumor_o_largest_diameter (cm)`, `replacement_tum_ann_p` * `tumor_p_largest_diameter (cm)`
#  , na.rm = TRUE))
