...
fuDeath <- clinData %>% filter(!is.na(date_death)) %>% mutate(time_fu = as.double(difftime(date_death, date_liver_op, units = c("days"))), status_fu = 1)
fuAlive <- clinData %>% filter(is.na(date_death) & is.na(date_lost_follow_up)) %>% mutate(time_fu = as.double(difftime(dateEndFu, date_liver_op, units = c("days"))), status_fu = 0)
fuLost <- clinData %>% filter(!is.na(date_lost_follow_up)) %>% mutate(time_fu = as.double(difftime(date_lost_follow_up, date_liver_op, units = c("days"))), status_fu = 0)
