...
# Merge neoadj data
gp_probe <- merge(gp_probe, neoadj_data, by = "ids", all.x = TRUE)
...
# Merge neoadj data
gp_tumor <- merge(gp_tumor, neoadj_data, by = "ids", all.x = TRUE)
...
# Total number of tumors
length(unique(gp_slide$slide_name))
...
stopifnot(all_equal(gp_slide %>% group_by(ids) %>% filter(annotation_types == "D") %>% select(percent_gp), gp_slide %>% group_by(ids) %>% filter(annotation_types == "D") %>% select(percent_desmo_slide) %>% rename(percent_gp = percent_desmo_slide)))

# Merge neoadj data
gp_slide <- merge(gp_slide, neoadj_data, by = "ids", all.x = TRUE)
